...
from .models import Company, Person, PersonCompanyLink, PhoneNumber, EmailAddress, PostalAddress, WebLink
...
SCALARS = {"first_name": tr("Vardas"), "last_name": tr("Pavardė"), "job_title": tr("Pareigos"), "status": tr("Būsena")}
MULTIPLE = {
    "phones": (PhoneNumber, "number", tr("Telefonai"), "tel:"),
    "emails": (EmailAddress, "email", tr("El. paštas"), "mailto:"),
    "addresses": (PostalAddress, "address", tr("Adresai"), ""),
    "web_links": (WebLink, "url", tr("Nuorodos"), None),
}
...
def edit_contact_field(request, pk):
    ...
    try:
        if field in SCALARS:
            ...
            if name:
                company = Company.objects.filter(name__iexact=name, deleted_at__isnull=True).first()
                if company is None:
