...
- Foninius darbus (`extend_recurrences`, `send_notifications`, `fetch_mail`,
  `run_automations`, `deliver_webhooks`, `deactivate_inactive_users` — `accounts.py`, `purge_audit_log` — `audit.py`) vykdo `crm-worker` paslauga `compose.yaml` (ciklas kas
...
- `deploy/helm/crm/` — Helm chart'as Kubernetes'ui (web Deployment, migracijų Job
  kaip `pre-upgrade` hook, 7 CronJob'ai vietoj `crm-worker`, nginx Ingress;
